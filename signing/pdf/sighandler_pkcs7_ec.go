...
func NewAdobePKCS7Detached(privateKey interface{}, certificate *x509.Certificate) (model.SignatureHandler, error) {
	// TODO: Check ecdsa/rsa private key
	return &adobePKCS7Detached{
...
func (a *adobePKCS7Detached) InitSignature(sig *model.PdfSignature) error {
	...
	// Instead of calling Sign() to calculate to contents field size (commented out code below) we just reserve 8k.
	// Because we're signing out of band, an unnecessary call to Sign() is very unpractical.
...
func (a *adobePKCS7Detached) Sign(sig *model.PdfSignature, digest model.Hasher) error {
	logrus.Info("Sign() called for bytes")
	if a.emptySignature {
	...
	// TODO: Fix this
	//bogusPrivateKey := ecdsa.PrivateKey{
	//	PublicKey: *(a.privateKey.(crypto.Signer)).Public().(*ecdsa.PublicKey),
	//	D:         nil,
	//}
	...
	signedData.Detach()
	// Complete() to obtain the signature bytes
